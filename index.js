...
myObject[Symbol.iterator] = function () {
  ...
  return {
    next() {
      if (myObject.currentIndex < myObject.category.length) {
        return {
          done: false,
          value: myObject.currentIndex++,
        };
      } else if (myObject.currentIndex >= myObject.category.length) {
        return {
          done: true,
        };
      }
...
useSymbolIterator = function (obj) {
  let result = [];
  for (let num of obj.category) {
